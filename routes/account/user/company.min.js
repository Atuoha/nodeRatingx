'use strict';const express=require("express"),app=express(),router=express.Router(),Company=require("../../../models/Company"),User=require("../../../models/User"),Rating=require("../../../models/Rating"),faker=require("faker"),fs=require("fs"),{isEmpty}=require("../../../helpers/upload-helpers");router.all("/*",(a,d,b)=>{a.app.locals.layout="user";b()});router.get("/",(a,d)=>{Company.find().then(b=>{d.render("accounts/user/company",{title:"User|Company",companies:b})}).catch(b=>console.log(b))});
router.get("/subscribers_view",(a,d)=>{Company.find().then(b=>{d.render("accounts/user/company/subscribers_company_views",{title:"Companies|nodeRatings",companies:b})}).catch(b=>console.log(b))});router.get("/leaderboard",(a,d)=>{Company.find().sort({ratingSum:-1}).then(b=>{d.render("accounts/user/company/leaderboard",{title:"Leaderboard|nodeRatings",companies:b})}).catch(b=>console.log(b))});
...
router.get("/edit/:id",(a,d)=>{"Manager"!==a.user.role&&(a.flash("error_msg","Access Denied! Action requires managerial certifications ):"),d.redirect("/user/company"));Company.findOne({_id:a.params.id}).then(b=>{b.user.forEach(c=>{c!=a.user.id?(console.log("not recognised"),a.flash("error_msg","Access Denied! You can only edit companies you created ):"),d.redirect("/user/company")):(console.log("recognised"),d.render("accounts/user/company/edit",{title:"Company|Edit",company:b}))})}).catch(b=>console.log(b))});
router.get("/show/:id",(a,d)=>{Company.findOne({_id:a.params.id}).populate("user").then(b=>{Rating.find({company:a.params.id}).populate("company").populate("user").then(c=>{let e=Object.keys(c).length;d.render("accounts/user/company/show",{title:`${b.name}`,company:b,ratings:c,ratingCount:e,averageRating:parseFloat(b.ratingSum/b.ratingNumber,1)})}).catch(c=>console.log(c))}).catch(b=>console.log(b))});router.get("/search",(a,d)=>{d.render("home/search",{title:"Search|nodeRatings"})});
router.post("/search",(a,d)=>{let b=new RegExp(a.body.search,"i");Company.find({$or:[{name:b}]}).then(c=>{1<c.length?d.render("accounts/user/company/subscribers_company_views",{title:"Searched|nodeRatings",companies:c}):Company.findOne({$or:[{name:b}]}).populate("user").then(e=>{e||(a.flash("error_msg","No company with such name"),d.redirect("/search"));Rating.find({company:a.params.id}).populate("company").populate("user").then(f=>{let g=Object.keys(f).length;d.render("account/user/company/show",
{title:`${e.name}`,company:e,ratings:f,ratingCount:g,averageRating:parseFloat(e.ratingSum/e.ratingNumber,1)})}).catch(f=>console.log(f))}).catch(e=>console.log(e))}).catch(c=>console.log(c))});
...
router.put("/update/:id",(a,d)=>{"Manager"!==a.user.role&&(a.flash("error_msg","Access Denied! Action requires managerial certifications ):"),d.redirect("/user/company"));Company.findOne({_id:a.params.id}).then(b=>{let c=b.file;if(!isEmpty(a.files)){const e=a.files.file;c=Date.now()+"-"+e.name;e.mv("./public/uploads/"+c,f=>{f&&console.log(f)});"img_place.png"!==b.file&&""!==b.file&&fs.unlink("./public/uploads/"+b.file,f=>{f&&console.log(f)})}b.name=a.body.name;b.address=a.body.address;b.city=a.body.city;
b.country=a.body.country;b.sector=a.body.sector;b.website=a.body.website;b.file=c;b.save().then(e=>{a.flash("success_msg",`${e.name} has been updated :)`);d.redirect("/user/company")}).catch(e=>console.log(e))}).catch(b=>console.log(b))});
