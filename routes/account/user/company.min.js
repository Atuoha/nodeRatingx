...
router.get("/subscribers_view",(a,d)=>{Company.find().then(b=>{d.render("accounts/user/company/subscribers_company_views",{title:"Companies|nodeRatings",companies:b})}).catch(b=>console.log(b))});
...
router.get("/show/:id",(a,d)=>{Company.findOne({_id:a.params.id}).populate("user").then(b=>{Rating.find({company:a.params.id}).populate("company").populate("user").then(c=>{let e=Object.keys(c).length;d.render("accounts/user/company/show",{title:"Company|Show",company:b,ratings:c,ratingCount:e,averageRating:parseFloat(b.ratingSum/b.ratingNumber,2)})}).catch(c=>console.log(c))}).catch(b=>console.log(b))});
...
c.country=a.body.country;c.sector=a.body.sector;c.website=a.body.website;c.file=b;c.user.push(a.user);c.save().then(e=>{User.findOne({_id:a.params.id}).then(f=>{f.company=c;f.save().then(g=>{a.flash("success_msg","Company registered successfully");d.redirect("/user/company")}).catch(g=>console.log(g))})}).catch(e=>console.log(e))}}).catch(b=>console.log(b))});
