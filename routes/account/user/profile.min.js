'use strict';const express=require("express"),app=express(),router=express.Router(),User=require("../../../models/User"),Company=require("../../../models/Company"),fs=require("fs"),bcrypt=require("bcryptjs"),{isEmpty}=require("../../../helpers/upload-helpers");router.all("/*",(b,d,a)=>{b.app.locals.layout="user";a()});router.get("/",(b,d)=>{User.findOne({_id:b.user.id}).populate("company").then(a=>{d.render("accounts/user/profile",{title:"Profile|User",profile:a})}).catch(a=>console.log(a))});
router.get("/edit/:id",(b,d)=>{User.findOne({_id:b.params.id}).then(a=>{Company.find().then(c=>{d.render("accounts/user/profile/edit",{title:"Edit|Profile",profile:a,companies:c})}).catch(c=>console.log(c))}).catch(a=>console.log(a))});
router.get("/edit/:id/company/:comp_id",(b,d)=>{User.findOne({_id:b.params.id}).populate("company").then(a=>{Company.findOne({_id:b.params.comp_id}).populate("company").then(c=>{d.render("accounts/user/profile/edit",{title:"Edit|Profile",profile,company:c})}).catch(c=>console.log(c))}).catch(a=>console.log(a))});
